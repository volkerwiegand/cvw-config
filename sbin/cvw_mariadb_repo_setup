...
then
    ...
    # for RHEL/CentOS 8
     case $os_version in
       8*) extra_options="module_hotfixes = 1" ;;
     esac
...
baseurl = https://dlm.mariadb.com/repo/maxscale/%s/rhel/$releasever/$basearch
...
# To use the latest stable release of MaxScale, use "latest" as the version
# To use the latest beta (or stable if no current beta) release of MaxScale, use "beta" as the version
deb https://dlm.mariadb.com/repo/maxscale/%s/%s %s main'
...
# To use the latest beta (or stable if no current beta) release of MaxScale, use "beta" as the version
name = MariaDB MaxScale
baseurl = https://dlm.mariadb.com/repo/maxscale/%s/sles/%s/$basearch
gpgkey = file:///etc/pki/rpm-gpg/MariaDB-MaxScale-GPG-KEY
enabled = 1
autorefresh=1
gpgcheck = 1
...
case $os_type in
    ...
    rhel)
        {
            ((skip_server)) || printf "$rhel_repo_server\n\n" "$mariadb_server_version" "$extra_options"
